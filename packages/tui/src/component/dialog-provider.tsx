...
export function providerOptions(list: { id: string; name: string }[]): ProviderOption[] {
  return pipe(
    ...
    map((provider) => ({
      ...
      description: {
        komocode: "(Recommended)",
      }[provider.id],
      category: "KomoCode Models",
    })),
  )
}
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in komocode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          komocode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                KomoCode gives you access to powerful AI coding models with a single API key.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://komocode.dev</span> to get your key
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in komocode.json to use it.`,
          })
          dialog.clear()
          return
        }

...
const _KOMO_LOG = join(process.env["TEMP"] ?? process.env["TMP"] ?? homedir(), "komocode-debug.log")
function _dlog(label: string, data?: unknown) {
  try {
    const line = `[${new Date().toISOString()}] PROMPT:${label} ${JSON.stringify(data ?? {})}\n`
    appendFileSync(_KOMO_LOG, line, "utf-8")
  } catch {}
}
...
export function Prompt(props: PromptProps) {
  ...
  onMount(() => {
    _dlog("PROMPT_MOUNTED", {
      agents: sync.data.agent.map((a) => a.name),
      providers: sync.data.provider.map((p) => p.id),
  ...
  async function submitInner() {
    ...
    _dlog("SUBMIT_CALLED", {
      ...
      wsCreating: workspace.creating(),
      moveCreating: move.creating(),
      autoVisible: !!auto()?.visible,
      inputLen: store.prompt.input?.length ?? 0,
      agents: sync.data.agent.map((a) => a.name),
      providers: sync.data.provider.map((p) => p.id),
      currentAgent: local.agent.current()?.name,
      currentModel: local.model.current(),
    })
    if (props.disabled) { _dlog("SUBMIT_BLOCKED", { reason: "disabled" }); return false }
    if (workspace.creating() || move.creating()) { _dlog("SUBMIT_BLOCKED", { reason: "workspace/move creating" }); return false }
    if (auto()?.visible) { _dlog("SUBMIT_BLOCKED", { reason: "autocomplete visible" }); return false }
    if (!store.prompt.input) { _dlog("SUBMIT_BLOCKED", { reason: "no input" }); return false }
    const agent = local.agent.current()
    if (!agent) { _dlog("SUBMIT_BLOCKED", { reason: "no agent", agentList: sync.data.agent.map((a) => a.name) }); return false }
    ...
    if (!selectedModel) {
      _dlog("SUBMIT_BLOCKED", { reason: "no model", providers: sync.data.provider.map((p) => ({ id: p.id, models: Object.keys(p.models) })) })
      void promptModelWarning()
      return false
    }
    _dlog("SELECTED_PROVIDER_MODEL", { providerID: selectedModel.providerID, modelID: selectedModel.modelID, agent: agent.name })
    ...
    _dlog("SESSION_EXISTS", { sessionID: sessionID ?? null })
    ...
    if (sessionID == null) {
      ...
      _dlog("SESSION_CREATING", { directory, workspaceID, agent: agent.name, model: selectedModel })
      ...
      if (res.error) {
        ...
        _dlog("SESSION_CREATE_ERROR", { error: String(res.error), errorJSON: JSON.stringify(res.error) })
      ...
      _dlog("SESSION_CREATED", { sessionID })
    }
    ...
    if (store.mode === "shell") {
      ...
      move.startSubmit()
      _dlog("SESSION_PROMPT_CALLED", { sessionID, provider: selectedModel.providerID, model: selectedModel.modelID })
      sdk.client.session
        .prompt(
          {
            sessionID,
        ...
        .then(() => { _dlog("SESSION_PROMPT_OK", { sessionID }) })
        .catch((error) => {
          _dlog("SESSION_PROMPT_ERROR", { sessionID, error: String(error) })
          toast.show({
          ...
          >
            <textarea
              ...
              onSubmit={() => {
                ...
                const rawText = input?.plainText ?? store.prompt.input
                const masked = rawText.length > 0 ? rawText.slice(0, 4) + "***" : "(empty)"
                _dlog("ENTER_KEY_RECEIVED", { text: masked, len: rawText.length, submitting })

...
// ── KOMOCODE DEBUG LOGGING ────────────────────────────────────────────────────
function dlog2(label: string, data?: unknown) {
  try {
...
import { DialogKomocodeLogin } from "./component/dialog-komocode-login"
            ...
            >
              <EpilogueProvider set={(value) => (exit.epilogue = value)}>
                <ErrorBoundary fallback={(error, reset) => {
                  dlog2("ROOT-ERROR-BOUNDARY", { msg: String(error), stack: error instanceof Error ? error.stack : "" })
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  // ── TRACE: App mounted ─────────────────────────────────────────────────────
  onMount(() => dlog2("APP-MOUNTED", {}))

  const [ready, setReady] = createSignal(false)

  // ── TRACE: signal watchers ─────────────────────────────────────────────────
  createEffect(() => dlog2("TRACE-READY", { ready: ready() }))
  createEffect(() => dlog2("TRACE-SYNC-STATUS", { status: sync.status }))
  createEffect(() => dlog2("TRACE-ROUTE", { type: route.data.type }))
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("KomoCode")
      return
    }
  ...
  createEffect(
    on(
      ...
      (isEmpty, wasEmpty) => {
        ...
        if (isKomocode) {
          dialog.replace(() => <DialogKomocodeLogin />)
          return
        }
  ...
  createEffect(() => {
    ...
    const diag = {
      route: route.data,
      providers: sync.data.provider.map((p: any) => p.id),
      provider_default: sync.data.provider_default,
      model: local.model.current(),
      ready: ready(),
    }
    console.log("[KOMOCODE-DIAG]", JSON.stringify(diag))
    try {
      const { appendFileSync } = require("fs") as typeof import("fs")
      const logPath = (process.env["TEMP"] ?? process.env["TMP"] ?? require("os").homedir()) + "/komocode-debug.log"
      appendFileSync(logPath, `[${new Date().toISOString()}] DIAG ${JSON.stringify(diag)}\n`, "utf-8")
    } catch {}
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://komocode.dev/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to KomoCode v${result.data.version}. Please restart the application.`,
    )
  ...
  // TraceMount: logs when SolidJS mounts this component (fires on Show/Match enter)
  function TraceMount(p: { label: string }) {
    onMount(() => dlog2("TRACE-MOUNT", { label: p.label }))
    return null as any
  }
  ...
  return (
    ...
    >
      ...
      <Show when={ready()}>
        <TraceMount label="show-ready-rendered" />
        <box flexGrow={1} minHeight={0} flexDirection="column">
          <TraceMount label="before-switch" />
          <Switch>
            <Match when={route.data.type === "home"}>
              <box flexGrow={1} flexDirection="column">
                <ErrorBoundary fallback={(err) => {
                  dlog2("HOME-ERROR-BOUNDARY", { msg: String(err), stack: err instanceof Error ? err.stack : "" })
                  return <text fg="red">HOME ERROR: {String(err)}</text>
                }}>
                  <Home />
                </ErrorBoundary>
              </box>

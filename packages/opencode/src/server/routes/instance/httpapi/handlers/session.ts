...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const _komo_dlog = (msg: string) => {
      try {
        appendFileSync(_komo_logPath, `[${new Date().toISOString()}] SERVER:${msg}\n`, "utf-8")
      } catch {}
    }
    ...
    const createRaw = Effect.fn("SessionHttpApi.createRaw")(function* (ctx: {
      ...
      yield* Effect.sync(() => _komo_dlog(`createRaw body=${body.slice(0, 500)}`))
      if (body.trim().length === 0) return yield* create({})

      const json = yield* tryParseJson(body).pipe(
        Effect.tap((j) => Effect.sync(() => _komo_dlog(`createRaw json=${JSON.stringify(j)}`))),
        Effect.tapError((e) => Effect.sync(() => _komo_dlog(`createRaw JSON_PARSE_FAIL ${String(e)}`))),
      )
      const decoded = yield* Schema.decodeUnknownEffect(Session.CreateInput)(json).pipe(
        Effect.tapError((e) => Effect.sync(() => _komo_dlog(`createRaw SCHEMA_FAIL ${JSON.stringify(e)}`))),

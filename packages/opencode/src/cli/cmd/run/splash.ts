...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  // Full KOMOCODE logo: KOMO (left, muted) + CODE (right, bright), rows 1-3
  const komo = logo.left.slice(1)
  const code = logo.right.slice(1)
  const leftWidth = logo.left[0]?.length ?? 0   // 21
  const rightWidth = logo.right[0]?.length ?? 0  // 19
  const markWidth = leftWidth + 1 + rightWidth   // 41
  const markRows = komo.length                   // 3

  function drawLogo(top: number) {
    for (let i = 0; i < markRows; i++) {
      draw(lines, komo[i] ?? "", { left: 0, top: top + i, fg: left, shadow: leftShadow })
      draw(lines, code[i] ?? "", { left: leftWidth + 1, top: top + i, fg: right, shadow: leftShadow })
    }
  }

  if (kind === "entry") {
    const top = 1
    const body_left = markWidth + 2
    drawLogo(top)
    push(lines, body_left, top, "KomoCode", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
